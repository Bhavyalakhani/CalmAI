...
def test_preprocess_entries_callable(mock_config, mock_settings):
    """Ensure the preprocess_entries_callable runs and pushes preprocessed journals to xcom"""
    mock_config.settings = mock_settings
    from dags.incoming_journals_pipeline import preprocess_entries_callable

    journals = [
        {"journal_id": "j1", "patient_id": "p1", "content": "Today was okay.", "entry_date": "2025-01-01"}
    ]

    ti = MagicMock()
    def xcom_pull(task_ids, key=None):
        if task_ids == "fetch_new_entries" and key == "incoming_journals":
            return journals
        return None
    ...
    context = {"ti": ti}
    preprocess_entries_callable(**context)

    assert "preprocessed_journals" in pushed
    assert pushed.get("preprocessed_count") == 1


@patch("storage.mongodb_client.MongoDBClient")
def test_fetch_pushes_duration(MockClient):
    """fetch_new_entries_callable should push a duration XCom when it runs"""
    from dags.incoming_journals_pipeline import fetch_new_entries_callable

    mock_client = MagicMock()
    mock_client.connect = MagicMock()
    mock_client.fetch_unprocessed_journals.return_value = [
        {"_id": "oid1", "journal_id": "j1", "content": "entry"}
    ]
    ...
    res = fetch_new_entries_callable(**context)
    assert res is True
    assert "duration" in pushed
    assert isinstance(pushed["duration"], float) or isinstance(pushed["duration"], int)


def test_start_pushes_run_id_and_duration():
    """start_callable should push run_id and a duration (0)"""
    from dags.incoming_journals_pipeline import start_callable
    ti = MagicMock()
    pushed = {}
    def xcom_push(key, value):
        pushed[key] = value
    ...
    ti.xcom_push.side_effect = xcom_push
    # do not provide a dag_run — start_callable generates the run_id itself
    context = {"ti": ti}
    start_callable(**context)

    assert "run_id" in pushed
    # run_id format: YYYYMMDD_HHMMSS (e.g. 20250220_000000)
    assert isinstance(pushed.get("run_id"), str)
    assert len(pushed.get("run_id")) == 15
    assert pushed.get("duration") == 0
